...
template<typename T>
KDTreeCube fill_and_bound_3d_kd_tree(const pandora::Algorithm* const  caller,
				     const std::unordered_set<const T*>& points,
...
template<typename T>
KDTreeCube fill_and_bound_3d_kd_tree(const std::unordered_set<const T*>& points,
				     std::vector<KDTreeNodeInfoT<const T*,3> >& nodes) {
  std::array<float,3> minpos{ {0.0f,0.0f,0.0f} }, maxpos{ {0.0f,0.0f,0.0f} };
  unsigned i = 0;
  for( const T *const  point : points ) {
